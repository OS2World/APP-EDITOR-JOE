...
/* Load termcap entry */
/* CAP *getcap(name,baud,out,outptr) */  /* changed p.w. wong */
...
if(!name && !(name=getenv("JOETERM"))) return 0;
...
tp=getenv("JOETERMCAP");

if(tp && tp[0]=='/') namebuf=vsncpy(NULL,0,sz(tp));
else
 {
 if(tp) vsncpy(sv(cap->tbuf),sz(tp));
 if((tp=getenv("JOETERMCAP"))) namebuf=vsncpy(NULL,0,sz(tp));
...
char *buf, *name;
{
/* latest=getcap(name,9600,stupid,NULL); */ /* changed p.w. wong */
